  ...
  if (isUser) {
    return (
      <div
        ...
        style={{ background: T.ink, color: T.inkOnBrand }}
      >
        <p className="whitespace-pre-wrap text-base leading-6">{text}</p>
        {time && (
          <p className="mt-1.5 text-xs" style={{ color: T.inkOnBrandMuted }}>
            {time}
          </p>
        )}
      </div>
  ...
  return (
    <div className="rounded-2xl px-4 py-3" style={{ background: T.card }}>
      <div className="mb-1.5 flex items-center gap-2">
        ...
        <span className="text-sm" style={{ color: T.ink, fontWeight: 600 }}>
          Mr Ducky
        </span>
      </div>
      <div
